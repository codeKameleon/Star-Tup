...
export default function User() {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_API}/users/${cookies.jwt}`, { headers: { withCredentials: true } })
            .then(res => {
                setUser(res.data)
            })
    ...
    function logout() {
        axios.get(`${process.env.REACT_APP_API}/account/logout`, { headers: { withCredentials: true } })
            .then(res => {
                removeCookie('jwt')
                navigate("/")
            })
